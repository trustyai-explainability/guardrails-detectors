...
class TestPerformance:
    ...
    def test_concurrent_evaluations_content(self) -> None:
        """Test handling multiple concurrent evaluations for content analysis."""
    ...
    def test_batch_processing_performance_content(self) -> None:
        """Test performance of batch processing for content analysis."""
        with patch.dict(os.environ, {"VLLM_BASE_URL": "http://test:8000"}):
            with patch('vllm_judge.Judge.from_url') as mock_judge_class:
                mock_judge = AsyncMock()
    ...
    def test_concurrent_evaluations_generation(self) -> None:
        """Test handling multiple concurrent evaluations for generation analysis."""
        with patch.dict(os.environ, {"VLLM_BASE_URL": "http://test:8000"}):
            with patch('vllm_judge.Judge.from_url') as mock_judge_class:
                # Mock judge with slight delay to simulate real processing
                mock_judge = AsyncMock()
                ...
                async def run_concurrent_evaluations():
                    ...
                    tasks = []
                    for content in contents:
                        task = detector.evaluate_single_generation(
                            prompt=content,
